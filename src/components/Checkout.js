  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        <Button
          variant="contained"
          onClick={async (e) => { await addAddress(token, newAddress.value) }}
        >
          Add
        </Button>
        <Button
...
const Checkout = () => {
    ...
    if (validateRequest(items, addresses)) {
      try {
        await axios({
          ...
          data: { addressId: addresses.selected },
          headers: { "Content-type": "application/json", "Authorization": `Bearer ${localStorage.getItem("token")}` }
        });
        enqueueSnackbar(
  ...
  useEffect(() => {
    const onLoadHandler = async () => {
      if (!token) {
        enqueueSnackbar('You must be logged in to access checkout page', { autoHideDuration: 3000, variant: 'error' });
        history.push('/')
      }
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            {newAddress.isAddingNewAddress ? null : <Button
              color="primary"
              variant="contained"
              id="add-new-btn"
              size="large"
            ...
            {!newAddress.isAddingNewAddress ? null : <AddNewAddressView
              token={token}
